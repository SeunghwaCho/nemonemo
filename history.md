...
## 2026-04-21 - 기능 추가: 일시정지/이어하기 지원

### 작업 내용
- `GameScene.ts`에 일시정지(⏸) 버튼 추가 — 헤더 다시하기 버튼 왼쪽
- `isPaused` 상태 추가: 일시정지 중 타이머 정지, 그리드 조작 차단
- `renderPauseOverlay()`: 반투명 오버레이에 이어하기/다시하기/목록으로 버튼 표시
- `pause()` / `resume()` 메서드: 타이머 elapsed 정확하게 보존
...
### 작업 내용
- 메뉴 화면 스크롤바 렌더링에서 잘못된 상수 참조로 발생하던 런타임 오류 수정
- 메뉴 렌더링 회귀 테스트 추가로 동일 오류 재발 방지
